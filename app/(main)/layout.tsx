                        ...
                        <ul>
                            {navItems.map(({ href, label, icon: Icon }) => (
                                <li key={href}
                                    className=''
                                >
                                    <Link href={href} className={`flex items-center space-x-2 p-4 text-gray-700 hover:bg-gray-100 ${path === href ? "bg-blue-100" : ""}`}>
                ...
                <main className="flex-1 overflow-y-auto p-4 md:p-8">
                    <header className="flex justify-between items-center mb-4">
                        <h2 className="text-5xl md:text-6xl gradient-title pt-2 md:pt-0 text-center md:text-left w-full">
                            {navItems.find((item) => item.href === path)?.label ||
                                "Dashboard"}
                        </h2>
                    </header>
                    {children}
                    ...
                    <ul className="flex justify-around">
                        {navItems.map((item) => (
                            <li key={item.href}>
                                <Link
                                    href={item.href}
                                    className={`flex flex-col items-center py-2 px-4 ${path === item.href ? "text-blue-600" : "text-gray-600"
                                        }`}
                                >
